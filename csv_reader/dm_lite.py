...
filename = "../ev_data/temp.csv"
log = "../ev_data/car_data.csv"

# as the program continues to run, the CSV file grows larger
# and lags the JS program. There are now two logs: one which
# stores a complete record and one which stores the last
# maxRows rows. The JS file keeps its own record separately
# as to record more than 32 points without dealing with huge
